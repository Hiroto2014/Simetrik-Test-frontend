...
class GeneralComponents(object):
    ...
    def wait_until_url_is(self, url, timeout=10):
        try:
            return WebDriverWait(self.web_driver, timeout).until(
                EC.url_contains(url),  # Usamos url_contains en lugar de url_to_be
                f"The page does not contain '{url}' after {timeout} seconds"
            )
